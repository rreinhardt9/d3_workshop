...
(function() {
  ...
  $(function() {
    ...
    return draw(".baseline-plot svg", data);
  });
  ...
  draw = function(container, data) {
    ...
    model = new Baseline;
    chart = model.build();
    $(".baseline-value, .current-value").keydown(function(e) {
      if (e.keyCode === 13) {
        return updateChart();
      }
    });
    $(container).empty();
    return d3.select(container).datum(data).transition().duration(500).call(chart);
  };

  this.updateChart = function() {
    var baselineVal, currentVal, data;
    ...
    Baseline.prototype.build = function() {
      ...
      return chart = function(selection) {
        return selection.each(function(d, i) {
          ...
          xScale = d3.scale.linear().domain(scaleDomain()).range([0, availableWidth]);
